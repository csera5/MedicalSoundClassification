# Onehot encoding for coldPresent, noise for cough data, age, and packYears
...
def load_data(testing=False):
    ...
    for i in range(Xtrain.shape[0]):
        ...
        newXtrain = np.concatenate((newXtrain, np.concatenate((cough, np.atleast_2d(Xtrain[i, 1:8]).astype(float), np.atleast_2d(onehot_train_coldpresent[i]), np.atleast_2d(Xtrain[i, 9]).astype(float)), axis=1)), axis=0, dtype=float)

    newXtest = np.zeros((0, ENCODING + 11))
    for i in range(Xtest.shape[0]):
        cough = np.load(f"sounds/sounds/{Xtest[i, 0]}/cough-opera.npy")
        newXtest = np.concatenate((newXtest, np.concatenate((cough, np.atleast_2d(Xtest[i, 1:8]).astype(float), np.atleast_2d(onehot_test_coldpresent[i]), np.atleast_2d(Xtest[i, 9]).astype(float)), axis=1)), axis=0, dtype=float)
    ...
    newXtrain = np.concatenate((newXtrain, np.concatenate((newXtrain[:, :ENCODING] + cough_noise, np.atleast_2d(newXtrain[:, ENCODING]).T + age_noise, newXtrain[:, ENCODING + 1:-1], np.atleast_2d(newXtrain[:, -1]).T + packYears_noise), axis=1)), axis=0)
